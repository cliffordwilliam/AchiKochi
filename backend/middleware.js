...
module.exports = class Middleware {
        ...
        switch (err.name) {
            case "SequelizeValidationError":
                return res
                    .status(400)
                    .json({
                        error: { msg: err.errors[0].message, status: 400 },
                    });
            ...
            case "JsonWebTokenError":
                return res
                    .status(401)
                    .json({ error: { msg: err.message, status: 401 } });
            case "CustomError":
                return res
                    .status(err.status)
                    .json({ error: { msg: err.msg, status: err.status } });
            default:
                return res
                    .status(500)
                    .json({
    ...
    static async tokenGuard(req, res, next) {
        try {
            ...
            const user = await Helper.findOne(
                User,
                { username: payload.username },
                "Unauthorized. Your access token is invalid.",
                401
            );
    ...
    static checkCredit(req, res, next) {
        if (req.loggedInUser.credit < 50)
            Helper.customError("Insufficient credit. Access forbidden.", 403);
        next();
    }
};
