...
module.exports = class ChatController {
    ...
    static async get(req, res, next) {
        try {
            ...
            chat_room_id = ChatController.checkChatRoomId(chat_room_id);
            // findById, exists?
            const foundChatRoom = await Helper.findById(
                chat_room_id,
                Chat_Room,
                `Chat room not found. No chat room with the ID ${chat_room_id} exists.`,
                404
            );
    ...
    static async patch(req, res, next) {
        try {
            ...
            // findById, exists?
            const foundChat = await Helper.findById(
                id,
                Chat,
                `Chat not found. No chat with the ID ${id} exists.`,
                404
            );
            // patch
            const [count, [patchedChat]] = await Chat.update(
                { status: "Read" },
                { where: { id }, returning: true }
            );
            const chatWithUser = await Chat.findOne({
                where: { id: patchedChat.id },
                include: [
                    { model: User, attributes: { exclude: ["password"] } },
                    { model: Chat_Room },
                ],
